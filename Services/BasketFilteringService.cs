...
namespace Basket.Filter.Services
{
    public class BasketFilteringService : IBasketFilteringService
    {
        ...
        private readonly ICatalogService _catalogService; // ← ADD THIS
        ...
        public async Task<BasketFilteringResponse> FilterBasketAsync(BasketRequest request)
        {
            ...
            var fees = request.AdditionalCharges?.Select(c => new Fee
            {
                Type = c.ChargeType,
                Name = c.ChargeName,
                Amount = (decimal)c.ChargeAmount / 100,
            ...
            return new BasketFilteringResponse
            {
                ...
                IsFullyEligible = ineligibleAmount == totalFeesAmount,
                ReasonIfNotEligible = ineligibleAmount > totalFeesAmount ? "Contains non-food items or alcohol" : "Delivery fees excluded"
            };
        }
